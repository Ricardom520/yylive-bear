...
class Canvas {
  ...
  private canvasRender() {
    ...
    function fillGradientColor(
      ...
      const gradient = context.createLinearGradient(x, y, r1, r2)
  ...
  private drawChatBox(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number) {
    ...
    ctx.quadraticCurveTo(x + 0.38 * w, y + 0.8 * h, x + 0.38 * w, y + 0.72 * h)
    ctx.quadraticCurveTo(x, y + 0.7 * h, x, y + 0.35 * h)
    ctx.closePath()
    ctx.stroke()
